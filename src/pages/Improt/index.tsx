...
import { ImportHeaders } from './column/medicines';
  ...
  const handleDeleteMedicine = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/medicines/${selectedMedicineId}`,
        {
          method: 'DELETE',
        }
      );
      ...
      setMedicines((prevMedicines) =>
        prevMedicines.filter((medicine) => medicine.medicine_id !== selectedMedicineId)
      );
      setShowModal(false);
  ...
  return (
    <div className="rounded bg-white pt-4 dark:bg-boxdark">
      <div className="flex items-center justify-between border-b border-stroke px-4 pb-4 dark:border-strokedark">
        <h1 className="text-md md:text-lg lg:text-xl font-medium text-strokedark dark:text-bodydark3">ຈັດການຂໍ້ມູນຢາ ແລະ ອຸປະກອນ</h1>
              ...
              {filteredMedicines.length > 0 ? (
                filteredMedicines.map((medicine, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">{medicine.status}</td>
                    <td className="px-4 py-4">
                      {(medicine.price * 1).toLocaleString()}
                    </td>
                    <td className="px-4 py-4">
                      {new Date(medicine.expired).toLocaleDateString('th-TH', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                      })}
